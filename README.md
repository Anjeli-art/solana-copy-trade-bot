...
- Mirror worker: polls one mirror source wallet and copies buy and sell direction.
...
flowchart LR
  ...
  subgraph fullcopy [Mirror Worker]
    MRPC[HTTP RPC polling] --> MDetect[platformDetector]
    MDetect --> MBuy[Jupiter Mirror Buy]
    MDetect --> MSell[Jupiter Mirror Sell]
  end
...
Mirror mode copies one source wallet through the same RPC polling and Jupiter execution stack.

1. Add one mirror wallet in the Mirror page.
2. Set `SOL per buy`.
3. Click `Start mirror`.
4. The mirror worker polls source wallet signatures through `MAINNET_ENDPOINT` or `RPC_ENDPOINT`.
5. If the source wallet buys a supported token, the bot opens a mirror position through Jupiter.
6. If the source wallet sells a token that has an open mirror position, the bot sells the matching local position through Jupiter.
...
- Mirror worker poll: `MIRROR_TRADE_POLL_MS`, fallback `COPY_TRADE_POLL_MS`, default `2000`.
