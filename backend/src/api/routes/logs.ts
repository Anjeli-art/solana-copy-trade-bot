...
import { deleteAllBotLogs, deleteBotLog, deleteBotLogsByEvent, listBotLogEvents, listBotLogs } from "../services/logs";
...
export async function handleLogs(request: IncomingMessage, response: ServerResponse, id?: string) {
  ...
  if (request.method === "DELETE" && !id) {
    ...
    if (event) {
      const deleted = deleteBotLogsByEvent(event);
      sendJson(response, 200, { data: { deleted, event } });
    } else {
      const deleted = deleteAllBotLogs();
